...
export default function Record() {
    ...
    return (
        <div className="record">
            <List header="充值记录">
                {data.map((item, index) => (
                    <List.Item key={index}>时间：{moment((item.topup_Date)).format("YYYY-MM-DD HH:mm:ss") 
} <br/>充值金额： {item.topup_Amount_Kyat} Ks  充值手机：{item.topup_Phone} <br/> 状态：{item.topup_Order_State} </List.Item>
                ))}
